...
data "vsphere_network" "network_tenant_blue_1" {
  name          = var.network_tenant_blue_1
  datacenter_id = data.vsphere_datacenter.dc.id
...
resource "vsphere_virtual_machine" "vm20" {
  ...
  network_interface {
    network_id = data.vsphere_network.network_tenant_blue_1.id
  }
  cdrom {
    client_device = true
  } 
...
resource "null_resource" "vm20_configure" {
  depends_on = [vsphere_virtual_machine.vm20]
  provisioner "file" {
    connection {
      type        = "ssh"
      host        = vsphere_virtual_machine.vm20.default_ip_address
      insecure    = true
